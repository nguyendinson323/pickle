...
class MicrositeBuilderService {
  async createMicrosite(ownerId: number, micrositeData: CreateMicrositeRequest) {
    ...
    const existingSlug = await Microsite.findOne({ where: { subdomain } } as any);
    if (existingSlug) {
    ...
    const microsite = await Microsite.create({
      ownerId,
      ownerType: (owner.role === 'club' ? 'club' : 'state') as 'club' | 'partner' | 'state',
      name,
      // slug functionality replaced with subdomain
      subdomain,
      title: name,
  ...
  async updateMicrosite(micrositeId: number, ownerId: number, updateData: UpdateMicrositeRequest) {
    ...
    // Handle slug change (stored in settings)
    if (updateData.slug && updateData.slug !== ((microsite as any).settings?.slug)) {
      const existingSlug = await Microsite.findOne({
        where: { subdomain: updateData.slug, id: { [Op.ne]: micrositeId } }
      });
      if (existingSlug) {
        throw new Error('Slug already exists');
      }
  ...
  async deleteMicrosite(micrositeId: number, ownerId: number) {
    const microsite = await Microsite.findOne({
      where: { id: micrositeId, ownerId }
    });
    ...
    await MicrositeAnalytics.destroy({
      where: { micrositeId }
    });
  ...
  async duplicateMicrosite(micrositeId: number, ownerId: number, newSubdomain: string) {
    const originalMicrosite = await Microsite.findOne({
      where: { id: micrositeId, ownerId }
    });

    if (!originalMicrosite) {
      throw new Error('Microsite not found or access denied');
    }

    // Create a duplicate microsite
    const duplicateData = {
    ...
    // Remove the id and timestamps to create a new record
  ...
  async addPage(micrositeId: number, ownerId: number, pageData: AddPageRequest) {
    ...
    const currentPages = ((microsite as any).settings?.pages || []) as any[];
    const existingPage = currentPages.find(page => page.slug === pageData.slug);
    if (existingPage) {
    ...
    const newPage = {
      ...
      isPublished: false,
      sortOrder: currentPages.length
    };

    const updatedSettings = {
      ...(microsite as any).settings,
      pages: [...currentPages, newPage]
    };
    await microsite.update({ settings: updatedSettings });
  ...
  async updatePage(micrositeId: number, ownerId: number, pageId: string, pageData: UpdatePageRequest) {
    ...
    if (pageData.slug && pageData.slug !== currentPages[pageIndex].slug) {
      const existingPage = currentPages.find(page => page.slug === pageData.slug && page.id !== pageId);
      if (existingPage) {
        throw new Error('Page slug already exists');
      }
    }
  ...
  async uploadMedia(micrositeId: number, ownerId: number, file: any): Promise<MediaLibrary> {
    ...
    const media = await MediaLibrary.create({
      ...
      category: this.getCategoryFromMimeType(file.mimetype) as 'image' | 'video' | 'document' | 'audio',
      tags: [],
      usageCount: 0
    });
  ...
  async deleteMedia(micrositeId: number, ownerId: number, mediaId: number) {
    const microsite = await Microsite.findOne({
      where: { id: micrositeId, ownerId }
    });
  ...
  private async validateMicrosite(microsite: Microsite): Promise<{ isValid: boolean; errors: string[] }> {
    ...
    const pages = ((microsite as any).settings?.pages || []) as any[];
  ...
  private async generateSitemap(microsite: Microsite): Promise<void> {
    const baseUrl = `https://${microsite.subdomain}.pickleballfed.mx`;
